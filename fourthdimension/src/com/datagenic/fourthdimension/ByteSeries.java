...
/**
 * A ByteSeries is used to store byte data.  If you wish to store other datatypes consider using one of the
 * following series:<p>
 * 
 * FloatSeries<br>
 * LongSeries<br>
 ...
 * @see Series
 */
public class ByteSeries extends NumericSeries
{
     ...
     */
   public ByteSeries(String name, CalendarFrequency calendar)
   {
       super(name,calendar);
   }
   ...
   {
       return new DataPoint(indexList.get(position),listAdapter.getByte(position),statusList.get(position));
   }
   ...
   public Object clone()
   {
       ByteSeries series = new ByteSeries("Copy of " + this.getName(),this.getCalendar());
       ...
       series.listAdapter = ListAdapter.makeAdapter(listAdapter.toByteArray());
...
DataPointCollection ByteSeries.getDataPointCollectionImpl(long,long){
