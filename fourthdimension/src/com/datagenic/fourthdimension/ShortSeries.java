...
/**
 * A ShortSeries is used to store short data.  If you wish to store other datatypes consider using one of the
 * following series:<p>
 * 
 * FloatSeries<br>
 * LongSeries<br>
 ...
 * @see Series
 */
public class ShortSeries extends NumericSeries
{
     ...
     */
   public ShortSeries(String name, CalendarFrequency calendar)
   {
       super(name,calendar);
   }
   ...
   {
       return new DataPoint(indexList.get(position),listAdapter.getLong(position),statusList.get(position));
   }
   ...
   public Object clone()
   {
       ShortSeries series = new ShortSeries("Copy of " + this.getName(),this.getCalendar());
       ...
       series.listAdapter = ListAdapter.makeAdapter(listAdapter.toShortArray());
...
DataPointCollection ShortSeries.getDataPointCollectionImpl(long,long){
