...
import com.datagenic.fourthdimension.data.*;
...
import com.datagenic.fourthdimension.adapters.*;

/**
 * A DoubleSeries is used to store double data.  If you wish to store other datatypes consider using one of the
 * following series:<p>
 *
 * FloatSeries<br>
 * LongSeries<br>
 ...
 * @see Series
 */
public class DoubleSeries extends NumericSeries {
     ...
     */
    public DoubleSeries(String name, CalendarFrequency calendar) {
        super(name,calendar);
    }
    ...
    protected DataPoint getDataPointImpl(int position) {
        return new DataPoint(indexList.get(position),listAdapter.getDouble(position),statusList.get(position));
    }
     ...
     */
    public Object clone() {
        DoubleSeries series = new DoubleSeries("Copy of " + this.getName(),this.getCalendar());
        ...
        series.listAdapter = ListAdapter.makeAdapter(listAdapter.toDoubleArray());
